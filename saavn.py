...
def getHomePage():
    playlists_json = []
    respone = requests.get('https://www.jiosaavn.com/api.php?__call=playlist.getFeaturedPlaylists&_marker=false&language=tamil&offset=1&size=250&_format=json', verify=False)
    if respone.status_code == 200:
...
def downloadSongs(songs_json):
    ...
    for song in songs_json['songs']:
        ...
        try:
            location = os.path.join(os.path.sep, os.getcwd(), song['album'], filename)
